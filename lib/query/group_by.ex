defmodule QueryBuilder.Query.GroupBy do
  ...
  def group_by(ecto_query, assoc_list, expr) do
    group_by_exprs = build_group_by_exprs!(ecto_query, assoc_list, expr)
    Ecto.Query.group_by(ecto_query, ^group_by_exprs)
  end

  defp build_group_by_exprs!(ecto_query, assoc_list, expr) when is_list(expr) do
    ...
    expr
    |> Enum.flat_map(&build_group_by_exprs!(ecto_query, assoc_list, &1))
  end

  defp build_group_by_exprs!(ecto_query, assoc_list, expr) when is_function(expr, 1) do
    expr
    |> call_group_by_fun(ecto_query, assoc_list)
    |> build_group_by_exprs!(ecto_query, assoc_list)
  end

  defp build_group_by_exprs!(_ecto_query, _assoc_list, %Ecto.Query.DynamicExpr{} = dynamic),
    do: [dynamic]

  defp build_group_by_exprs!(ecto_query, assoc_list, token)
       when is_atom(token) or is_binary(token) do
    [token_to_dynamic(ecto_query, assoc_list, token)]
  end

  defp build_group_by_exprs!(_ecto_query, _assoc_list, expr) do
    raise ArgumentError,
          "group_by expects a token, a list of tokens/expressions, a dynamic, or a 1-arity function; got: #{inspect(expr)}"
  end

  defp call_group_by_fun(fun, ecto_query, assoc_list) do

defmodule QueryBuilder.Query.LeftJoinLatest do
  @moduledoc false
  ...
  def left_join_latest(%Query{} = ecto_query, %AssocList{} = assoc_list, assoc_field, opts) do
    %{
      order_by: order_by,
      where: where_filters,
    ...
    latest_binding = root_schema._latest_binding(assoc_field)

    if Query.has_named_binding?(ecto_query, latest_binding) do
      raise ArgumentError,
            "left_join_latest/3 attempted to use named binding #{inspect(latest_binding)} for " <>
              "#{inspect(root_schema)}.#{inspect(assoc_field)}, but that binding name is already present " <>
              "in the query. Fix: rename the conflicting binding (avoid `as: #{inspect(latest_binding)}`) " <>
              "or do not call left_join_latest for that association."
    end
    ...
    Query.select(ecto_query, [{^root_schema, p}, {^latest_binding, c}], {p, c})
  end
  ...
  defp validate_opts!(opts) when is_list(opts) do
    unless Keyword.keyword?(opts) do
      raise ArgumentError,
            "left_join_latest/3 expects `opts` to be a keyword list, got: #{inspect(opts)}"
    end
    ...
    if unknown_keys != [] do
      raise ArgumentError,
            "left_join_latest/3 got unknown options: #{inspect(unknown_keys)}. " <>
    ...
    if order_by in [nil, []] do
      raise ArgumentError, "left_join_latest/3 requires a non-empty `order_by:` option"
    end

    if is_nil(where) do
      raise ArgumentError,
            "left_join_latest/3 does not accept `where: nil`; omit the option or pass []"
    end

    unless is_list(where) do
      raise ArgumentError,
            "left_join_latest/3 expects `where:` to be a keyword list (or a list of filters), got: #{inspect(where)}"
    end

    if is_nil(child_assoc_fields) do
      raise ArgumentError,
            "left_join_latest/3 does not accept `child_assoc_fields: nil`; " <>
    ...
    KeyError ->
      raise ArgumentError,
            "left_join_latest/3 requires an `order_by:` option; got: #{inspect(opts)}"
  end

  defp validate_opts!(nil) do
    raise ArgumentError,
          "left_join_latest/3 expects `opts` to be a keyword list, got: nil"
  end

  defp validate_opts!(other) do
    raise ArgumentError,
          "left_join_latest/3 expects `opts` to be a keyword list, got: #{inspect(other)}"
  end
  ...
  defp fetch_has_many_assoc!(root_schema, assoc_field) when is_atom(assoc_field) do
    ...
    case assoc do
      ...
      %Ecto.Association.Has{cardinality: :one} ->
        raise ArgumentError,
              "left_join_latest/3 only supports has_many (to-many) associations; " <>
      ...
      %Ecto.Association.ManyToMany{} ->
        raise ArgumentError,
              "left_join_latest/3 does not support many_to_many associations; " <>
                "#{inspect(root_schema)}.#{inspect(assoc_field)} is many_to_many"

      %Ecto.Association.HasThrough{} ->
        raise ArgumentError,
              "left_join_latest/3 does not support has_many/has_one through associations; " <>
      ...
      other ->
        raise ArgumentError,
              "left_join_latest/3 does not support association #{inspect(root_schema)}.#{inspect(assoc_field)}; " <>
  ...
  defp fetch_has_many_assoc!(root_schema, assoc_field) do
    raise ArgumentError,
          "left_join_latest/3 expects `assoc_field` to be an association atom, got: #{inspect(assoc_field)} " <>
  ...
  defp validate_order_by!(order_by, assoc_field) do
    if is_nil(order_by) do
      raise ArgumentError,
            "left_join_latest/3 expects `order_by:` to be a keyword list for " <>
  ...
  defp validate_order_by_includes_primary_key!(order_by, assoc_schema)
    ...
    if primary_key_fields == [] do
      raise ArgumentError,
            "left_join_latest/3 requires the association schema to have a primary key so it can be used as " <>
    ...
    if missing != [] do
      raise ArgumentError,
            "left_join_latest/3 requires `order_by:` to include the association primary key fields as a tie-breaker; " <>

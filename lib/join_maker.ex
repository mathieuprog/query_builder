defmodule QueryBuilder.JoinMaker do
  ...
  defp do_make_joins(
    ...
    {ecto_query, nested_assocs} =
      if assoc_data.join_spec.joined? do
  ...
  defp maybe_join(
         ecto_query,
         %{join_spec: %JoinSpec{required?: false}} = assoc_data,
  ...
  defp maybe_join(ecto_query, assoc_data, original_assoc_list) do
    %{
      ...
      join_spec: %JoinSpec{qualifier: join_qualifier, filters: join_filters} = join_spec
    } = assoc_data
    ...
    if Ecto.Query.has_named_binding?(ecto_query, assoc_binding) do
      ...
      expected_qualifier = join_qualifier
      ...
      {ecto_query, %{assoc_data | join_spec: %{join_spec | joined?: true}}}
    else
      on =
        if join_filters != [] do
          join_filters
